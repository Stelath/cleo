...
class SaveVideoServicer(ToolServiceBase):
    ...
    def tool_input_schema(self) -> dict[str, Any]:
        return {
            ...
            "properties": {
                "past_seconds": {
                    "type": "number",
                    "description": "Seconds of past footage to record (default 30, max 30).",
                },
                "future_seconds": {
                    "type": "number",
                    "description": "Seconds of future footage to record after this command (default 45).",
                },
            },
            "required": [],
        }
    ...
    def execute(self, params: dict) -> tuple[bool, str]:
        past_s = min(
            float(params.get("past_seconds", SENSOR_AUDIO_BUFFER_SECONDS)),
            float(SENSOR_AUDIO_BUFFER_SECONDS),
        )
        future_s = float(params.get("future_seconds", 45.0))
